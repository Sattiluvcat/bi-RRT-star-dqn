...
from single_planning.dwa_res import dwa_plan
...
def run_rrt_dwa_times(run_times=100, environ=False):
    ...
    for i in range(run_times):
        ...
        try:
            ...
            path, rrt_path, stagnation_count, end_time = dwa_plan(start, goal, obstacle_list, environ)
            print("over-over")
            if path is not None:
    ...
    df.to_excel('dwa_results.xlsx', index=False)
    print("Results saved to dwa_results.xlsx")


if __name__ == "__main__":
    # set为True表示大障碍物环境，False表示L型障碍
    run_rrt_dwa_times(1, True)
    # start = [0, 5]
    # goal = [120, -95]
    # obstacle_list = [
    #     # 移动大障碍
    ...
    # plt.plot(goal[0], goal[1], "xk", label="goal")
    # plt.axis("equal")
    ...
    #     if len(obs) == 3:
    #         plot_obs(obs[0], obs[1], obs[2],"g",ss)
    #         ss+=1
    #     elif len(obs) == 4:
